...
date: 2025-10-25
summary: A Python console text adventure with a custom command parser, room/item system, inventory management, and save/load — the foundation for my Django-based Retro Adventure Game.
...
A terminal-based text adventure inspired by classics like Zork and Colossal Cave Adventure, built entirely in Python. Players explore interconnected rooms, collect items, solve puzzles, and manage inventory through a natural-language command parser. This project later evolved into the web-based Retro Adventure Game (Django).

## Role & Stack

Solo project using Python (standard library only — no external dependencies).
